...
export class MessageController {
  ...
  async sendVerificationCodeWhatsapp(
    @Body() body: { phone_number: string },
  ): Promise<any> {
    /** Gọi service để gửi mã xác minh qua Telegram. */
    const RESULT = await this.messageService.sendVerificationCodeWhatsapp(
      body.phone_number,
    );
  ...
  async verifyCodeWhatsapp(
    ...
    /** Gọi service để xác minh mã OTP và lấy session. */
    const RESULT = await this.messageService.verifyCodeWhatsapp(
      body.phone_number,
      body.phone_code,
    );
